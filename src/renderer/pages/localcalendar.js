    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      <div style="min-height:100px;background:${todayCell?'rgba(232,197,71,0.05)':'var(--surface)'};border:1px solid ${todayCell?'var(--accent)':'var(--border)'};padding:6px;cursor:pointer;transition:background 0.1s;position:relative"
        ...
        ${dayBlocks.slice(0,3).map(b => `
          <div onclick="openBlockModal(null,null,'${b.id}')" style="font-size:9px;padding:2px 5px;border-radius:2px;margin-bottom:2px;background:${roleColor(b.roleId)}22;border-left:2px solid ${roleColor(b.roleId)};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--text);cursor:pointer" title="Click to edit/delete: ${esc(b.title)}">
      ...
      ${Array.from({length:14},(_,h)=>h+7).map(hour => `
        ...
        ${days.map((d,di) => {
          ...
          <div style="border-right:1px solid var(--border);border-bottom:1px solid var(--border);min-height:44px;padding:2px;background:${isToday(d)?'rgba(232,197,71,0.03)':'transparent'};cursor:pointer;position:relative"
            ...
            ${hourBlocks.map(b=>`
              <div onclick="openBlockModal(null,null,'${b.id}')" style="font-size:9px;padding:2px 4px;border-radius:2px;margin-bottom:1px;background:${roleColor(b.roleId)}33;border-left:2px solid ${roleColor(b.roleId)};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--text);cursor:pointer" title="Click to edit/delete: ${esc(b.title)}">
